...
export default function Login() {
  ...
  // We'll manage alerts with these states
  ...
  const handleSubmit = async (e) => {
    ...
    // Reset alerts before new login attempt
    setSuccessAlert(false);
    setErrorAlert(false);
    setErrorMessage("");

    try {
      const response = await fetch("https://unused-item-donation.onrender.com/api/auth/login", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#1F1F1F] px-4">
      ...
      >
        <h1 className="text-green-400 text-4xl font-semibold text-center">Login</h1>
        <p className="text-gray-400 text-sm text-center mt-2">Please Sign in to continue</p>
        ...
        {/* Forgot Password */}
        <div className="mt-5 text-right text-green-400"
        ...
        <button
          type="submit"
          className="mt-6 w-full h-12 rounded-full text-[#1F1F1F] bg-[#B6F09C] hover:opacity-90 transition-opacity text-lg font-semibold"
        >
          Login
        </button>
